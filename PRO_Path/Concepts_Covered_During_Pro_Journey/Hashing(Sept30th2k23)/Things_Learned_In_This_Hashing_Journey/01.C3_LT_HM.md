...
## Problem 07 : [2309. Greatest English Letter in Upper and Lower Case](https://leetcode.com/problems/greatest-english-letter-in-upper-and-lower-case/description/)
...
-> I have used the Set since i need to calculate the Upper and Lower occurance so i have used Set as once i encountered lower case i reduced 32 and verified it the value exist if yes then push it into ordered set and at end return the last element of the orderd set.
...
public:
    string greatestLetter(string s) {
        ...
        if(ans.empty()) return "";
        return string (1,*(--ans.end()));
    }
};

```
Time Complexity : o(n) 24.14% 
Space Complexity : o(n+m)


-> Let's use the two separate arrays storing the frequency's of the upper and lower case.
...
public:
    string greatestLetter(string s) {
        int uc[26]={0},lc[26]={0},size=s.length();
        char ch;
        for(int i=0;i<size;++i)
        {ch=s[i];
            if(islower(ch))
        ...
        for(int i=25;i>=0;--i)
        {
            if(lc[i]>0 && uc[i]>0) return string(1,char(i+65));
        }
        return "";
    }
};
...
## Problem 08 : [888. Fair Candy Swap](https://leetcode.com/problems/fair-candy-swap/description/)
...
public:
    vector<int> fairCandySwap(vector<int>& aliceSizes, vector<int>& bobSizes) {
        ...
        // I want to exchange a value from each of the container's to equalize the sum between two container's 

        // So if there exist a value such that if we transfer one value from one container the sum between these 2 are in equalibrium this would be an invalid case.

        // So how could you find that value in both of the container's.

        // Let's consider two container's

        // [1,2,5] and [2,4]

        // Let's take sum of these two container's 8 and 6

        // let's take of these 2 sum's and find the mid point by dividing them by 2 -> 8+6/2 -> 14/2 -> 7

        // Nah this summation isn't working as i need to find the difference between 2 container's to bring it to the equalibrium 8-6/2 -> 1 now we have the difference value from container 1 to 2

        // Now what can i do with this difference let's say this is the difference that is needed to be added to the into one container and deduced from another

        // Since we obainted the value from A-B we cannot add the value to A as it will stengthen it so let's add it to B ans see that value exist in A 
        ...
        set<int>a(aliceSizes.begin(),aliceSizes.end());

        for(int i:bobSizes)
        {
            if(a.count(diff+i)!=0) return {diff+i,i};
        }
...
## Problem 09:[1331. Rank Transform of an Array](https://leetcode.com/problems/rank-transform-of-an-array/description/)
...
Space Complexity: 76.88%

Time Complexity: 46.59%


### Optimized:

1. Using Pair's and sorting them using custom operator to compare and overriding the original array with the updated values.
...
public:
    vector<int> arrayRankTransform(vector<int>& arr) {
        ...
        int size=arr.size();
        for(int i=0;i<size;++i){temp.push_back(make_pair(arr[i],i));}

        sort(temp.begin(),temp.end(),[](pair<int,int>&p1,pair<int,int>&p2){return p1.first<p2.first;});
        ...
        if(!temp.empty()) arr[temp.back().second]=ctr;
...
Space Complexity: 90.40%

Time Complexity: 96.99%

## Hyper Optimized 

1. Using the same above approach but i will update the result in another array and return the answer.
...
public:
    vector<int> arrayRankTransform(vector<int>& arr) {
        ...
        int ctr=1;
        ans[temp[0].second]=ctr;
        for(int i=1;i<size;++i)
        {
            if(temp[i].first==temp[i-1].first)
            {
                ans[temp[i].second]=ctr;
            }
            else
            {
                ans[temp[i].second]=++ctr;
...
## Problem 10:[2273. Find Resultant Array After Removing Anagrams](https://leetcode.com/problems/find-resultant-array-after-removing-anagrams/description/)
...
Space Complexity: 79.33%
Time Complexity: 49.60%
...
Time Complexity : 92.05%


## Problem 11 : [1640. Check Array Formation Through Concatenation](https://leetcode.com/problems/check-array-formation-through-concatenation/description/)
...
# The Idea was self originated from me about the code but not sure why i did receive buffer overflow error but with other's help this code was generated but the way the idea and thought's was placed into the code is literally stunning.
